...
В этой реализации используется вариант `3`: необходимые навыки проектов и фильтрация проектов по навыкам.
...
| **POSTGRES_PORT**     | Порт подключения к БД. В `docker-compose.yml` используется `5436`.                                                                                         |
...
Автотесты можно запустить на PostgreSQL после настройки `.env`:

```bash
python manage.py test
```

Для быстрой локальной проверки без PostgreSQL предусмотрен SQLite-режим:
...
Команда `seed_demo` создает трех тестовых пользователей, проекты для каждого из них, участников и набор навыков. Пароль всех демо-пользователей: `demo-pass-123`.

Реализовано: регистрация и вход по email, профили, редактирование профиля, смена пароля, список участников, создание/редактирование/завершение проектов, участие в проектах, необходимые навыки проектов, автодополнение навыков, фильтрация проектов по `?skill=` и пагинация списков.
